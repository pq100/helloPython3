...
# 이때 반환되는 값은 반환 값이라고 함
# 반환값은 함수 내부에서 return이라는 키워드로 지정

# 제곱값을 계산하는 함수
def squr(x):
    # 출력까지 함수에서 구현하기 때문에
    # 플랫폼이 변하는 경우 함수를 재 작성 해야하는 번거로움 발생!
    print(f'{x} => {x ** 2}')
